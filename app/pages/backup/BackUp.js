...
import { renderListItem } from './BackUpListItem';
...
export default class BackUp extends BaseComponent {
  ...
  componentDidMount() {
    // 加入通知监听
    this._deEmitter = DeviceEventEmitter.addListener(Constants.BackUpSearchBtnClickNotiName, (a) => {
      ...
      this.props.navigation.push('BackUpSearch', params);
    });
    // 加入StackNavigation监听
  ...
  _itemDidSelect = (item, index) => {
    ...
    params.data = item;
    this.props.navigation.push('BackUpDetail', params);
  }
  // 下拉刷新
  _onHeaderRefresh = () => {
    // 设置刷新状态
  ...
  _loadData = () => {
    ...
    let url = baseUrl + '/onsite/api/backup/SimensStaff/queryBackUpStaffPageList';
    var params = new Map();
