...
import { renderListItem } from './CheckOutListItem';
...
export default class CheckOut extends BaseComponent {
  ...
  componentDidMount() {
    // 加入通知监听
    this._deEmitter = DeviceEventEmitter.addListener(Constants.CheckOutSearchBtnClickNotiName, (a) => {
      ...
      this.props.navigation.push('CheckOutSearch', params);
    });
    // 加入StackNavigation监听
  ...
  _loadData = () => {
    ...
    let url = baseUrl + '/onsite/api/checkOut/SimensStaff/queryOutStaffPageList';
    var params = new Map();
